...
#ifndef SYMBOLICSETINTERFACE_HH_
#define SYMBOLICSETINTERFACE_HH_
...
public:
   ...
   */
   SymbolicSet(Cudd &ddmgr, const size_t dim, const double* lb, const double* ub, const double* eta, const double* z=NULL, bool GenerateNewBddVars=true) {
    ...
    for (size_t i=0; i<dim; i++) {
      ...
      nofGridPoints_[i]=(size_t)abs(Nu-Nl)+1;
      /* number of bits */
   ...
   * dimensions stored in vector<size_t> pdim 
   *
   ...
   *
   */
  SymbolicSet(const SymbolicSet& other, vector<size_t> pdim) {
    dim_=pdim.size();
    ddmgr_=other.ddmgr_;
    if(!dim_) {
      ostringstream os;
      os << "Error: SymbolicSet::SymbolicSet(const SymbolicSet& other, vector<size_t> pdim): projection indices pdim should be non-empty.";
      throw runtime_error(os.str().c_str());
    }
    ...
    if(oor) {
      ostringstream os;
      os << "Error: SymbolicSet::SymbolicSet(const SymbolicSet& other, vector<size_t> pdim): the elements in pdim need to be within [1...other.dim_].";
      throw runtime_error(os.str().c_str());
    }
    z_ = new double[dim_];
  ...
  void addByFunction(F &function) {
    ...
    BDD grid=SymbolicSet::computeGridPoints();
    /* set to be added */
  ...
  void addPolytope(const size_t p, const double* H, const double* h, const ApproximationType type) {
    BDD polytope=SymbolicSet::computePolytope(p, H, h, type);
  ...
  void addEllipsoid(const double* L, const double* c, const ApproximationType type) {
    BDD ellisoid=SymbolicSet::computeEllipsoid(L, c, type);
  ...
  void printInfo(int verbosity=0) const {
    ...
    cout << "First grid point: ";
    for(size_t i=0; i<dim_; i++) 
        cout << firstGridPoint_[i] << " ";
      cout << endl;
    if (verbosity) {
      cout << "Last grid point: ";
      for(size_t i=0; i<dim_; i++) 
          cout << lastGridPoint_[i] << " ";
        cout << endl;
    }


    cout << "Grid node distance (eta)in each dimension: ";
    for(size_t i=0; i<dim_; i++) 
      cout << eta_[i] << " ";
    cout << endl;

    cout << "Number of grid points in each dimension: ";
    for(size_t i=0; i<dim_; i++) 
      cout << nofGridPoints_[i] << " ";
    cout << endl;

    
    cout << "Number of grid points in domain: "<< totNoGridPoints << endl;
    if (symbolicSet_!=ddmgr_->bddZero())
      cout << "Number of elements in the symbolic set: "<< symbolicSet_.CountMinterm(totNoBddVars) << endl;
    else {
      cout << "Symbolic set does not contain any grid points."<< endl;
    }

    if (verbosity) {
      cout << "Number of binary variables: "<< totNoBddVars << endl;
      for(size_t i=0; i<dim_; i++) {
        cout << "Bdd variable indices (=IDs) in dim " << i+1 << ": ";
        for(size_t j=0; j<nofBddVars_[i]; j++) 
          cout << indBddVars_[i][j] << " ";
        cout << endl;
      }
    }
    if (verbosity>1)
  ...
  void writeToFile(const char *filename) {
    ...
    if (storeReturnValue!=DDDMP_SUCCESS) 
      ...
      cout << "Symbolic set saved to file: "<< filename << endl;
  } 
  /* function:  getZ
  ...
  void copyGridPoints(double gridPoints[]) const {
    ...
    while(!it.done()) {
      ...
      //for(size_t i=0; i<dim_; i++) 
      //  cout << gridPoints[i*nofMinterms+k] << " " ;
      //cout << endl;
      ++it; ++k;
    }
  }
   ...
   */
  void copyGridPoints(double gridPoints[], vector<size_t> projectDimension) const {
    /* check input data */
    size_t n=projectDimension.size();
    if(n>=dim_) {
      ostringstream os;
  ...
  */
   ...
   *
   */
  vector<vector<double>> setValuedMap(vector<double> x, vector<size_t> ind) {
    if(!(x.size()==ind.size())) {
      ostringstream os;
    ...
    for(size_t i=0; i<n; i++) {
      ...
      int *p=phase;
      int idx=lround((x[i]-firstGridPoint_[ind[i]])/eta_[ind[i]]);
      for (; idx; idx/=2) *(p++)=0+idx%2;
...
private:
  ...
  BDD computeEllipsoid(const double* L, const double* center, const ApproximationType type) {
    ...
    double r=SymbolicSet::computeEllipsoidRadius(L);
  ...
  double computeEllipsoidRadius(const double* L) {
    ...
    return sqrt(Cudd_V(max.getNode()));
  }
...
public:
	/* Extracts the bddVars from the given XUX SuymbolicSet*/
	static
	void ExtractBddVars(const SymbolicSet& ss_org, vector<size_t>& org_x_bddVars, vector<size_t>& org_u_bddVars, vector<size_t>& org_xd_bddVars, size_t SS_DIM, size_t IS_DIM){
		// Copying bddVars for X and X' from the original relation
		for(size_t i=0; i<SS_DIM; i++){
			size_t dim_nvars = ss_org.getNofBddVars()[i];
			for(size_t j=0; j<dim_nvars; j++){
				org_x_bddVars.push_back(ss_org.getIndBddVars()[i][j]);
				org_xd_bddVars.push_back(ss_org.getIndBddVars()[SS_DIM + IS_DIM + i][j]);
			}
		}

		// Copying bddVars for U from the original relation
		for(size_t i=0; i<IS_DIM; i++){
			size_t dim_nvars = ss_org.getNofBddVars()[SS_DIM + i];
			for(size_t j=0; j< dim_nvars; j++){
				org_u_bddVars.push_back(ss_org.getIndBddVars()[SS_DIM + i][j]);
			}
		}
	}
}; /* class SymbolicSetInterface */
